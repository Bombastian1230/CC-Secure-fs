...
local function keySender()
    while key do
        local event, sender, use = os.pullEvent("key_request")
        log(string.format("Key requested from %s for use in %s", sender, use))
        os.queueEvent("key_response", key)
    end
end
