-- This is just the normal cc tweaked editor but it encrypts the data before saving it
...
-- Encryption (added by me)
local chacha20 = require "chacha20"
local sha256 = require "sha256"
local utils = require "utils"

os.queueEvent("key request")
local event, key = os.pullEvent("key response")
local nonce = chacha20.generate_nonce()
...
local function encrypt_tLines()
    local e_data = {}
    local data = table.concat(tLines, "\n")
    local keystream = chacha20.generate_keystream(key, nonce, #table.concat(tLines))



    for i = 1, #data do
        e_data[i] = bit32.bxor(string.byte(data, i), keystream[i])
    end
    
    return e_data
end

local function save_encrypted(_sPath)
    local file, fileerr

    local nonce_bytes = table.pack(string.byte(nonce, 1, 12))
    local nonce_hexes = {}
    for _, byte in ipairs(nonce_bytes) do
    ...
    if file then
        if file then
            ...
            file.write(string.char(table.unpack(e_data)))
        end
    else
...
local function load_encrypted(_sPath)
    tLines = {}
    if fs.exists(_sPath) then
        local file = io.open(_sPath, "r")
        local e_data = file:read("a")
        
        local data = chacha20.crypt(e_data, key, nonce)
...
local function save(_sPath)
    ...
    local ok, err, fileerr = pcall(save_encrypted, _sPath)
    -- print(nonce, " ", key)
    -- if file then
    --     file.close()
    -- end
...
-- Actual program functionality begins, also modified by me
load_encrypted(sPath)
